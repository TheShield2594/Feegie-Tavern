...
export const FLAT_ZONES: FlatZone[] = [
  ...
  { x: 0, z: 0, radius: 17, falloff: 9, height: 2.6, surface: 'plaza' },
  ...
  { x: 0, z: -62, radius: 12, falloff: 9, height: 8.4, surface: 'grass' },      // town hall terrace
  { x: 52, z: -58, radius: 11, falloff: 10, height: 13.5, surface: 'rock' },    // lighthouse point
  { x: -52, z: -46, radius: 15, falloff: 12, height: 9.5, surface: 'grass' },   // high meadow
  { x: -40, z: 26, radius: 12, falloff: 8, height: 2.2, surface: 'dirt' },      // farm terrace
  { x: 14, z: 44, radius: 8, falloff: 7, height: 0.9, surface: 'sand' },        // harbour apron
];

export const PATHS: PathSegment[] = [
  // Square → cottage
  { ax: -6, az: -12, bx: -22, bz: -18, width: 3.4, surface: 'path' },
  // Square → shop
  { ax: 7, az: -9, bx: 19, bz: -12, width: 3.4, surface: 'path' },
  // Square → museum
  { ax: -3, az: -15, bx: -5, bz: -30, width: 3.6, surface: 'path' },
  { ax: -5, az: -30, bx: -6, bz: -38, width: 3.6, surface: 'path' },
  // Square → beach
  { ax: 2, az: 15, bx: 5, bz: 30, width: 3.8, surface: 'path' },
  { ax: 5, az: 30, bx: 11, bz: 41, width: 3.6, surface: 'path' },
  // Beach boardwalk east
  { ax: 11, az: 41, bx: 26, bz: 44, width: 3.0, surface: 'wood' },
  // Square → east residential
  { ax: 12, az: 4, bx: 32, bz: 4, width: 3.2, surface: 'path' },
  { ax: 32, az: 4, bx: 42, bz: 6, width: 3.0, surface: 'path' },
  // Square → west grove
  { ax: -12, az: -2, bx: -34, bz: -6, width: 3.2, surface: 'path' },
  { ax: -34, az: -6, bx: -44, bz: -8, width: 3.0, surface: 'path' },
  // Museum → town hall → lighthouse
  { ax: -6, az: -44, bx: -2, bz: -56, width: 3.2, surface: 'path' },
  { ax: 2, az: -62, bx: 24, bz: -60, width: 2.8, surface: 'path' },
  { ax: 24, az: -60, bx: 45, bz: -59, width: 2.6, surface: 'path' },
  // Farm spur
  { ax: -30, az: 12, bx: -38, bz: 22, width: 2.8, surface: 'dirt' },
  // Pip's house spur
  { ax: 24, az: -22, bx: 29, bz: -30, width: 2.6, surface: 'path' },
];
...
export const BRIDGES = [{ x: -42, z: 8, rotation: Math.PI / 2 - 0.28, width: 8, length: 4.5 }];
...
export function landMask(x: number, z: number): number {
  ...
  let radius = 86 + bays * 34 + detail * 7;
  // Pull the south shore in to make a proper wide beach and a sheltered cove.
  radius -= smoothstep(-0.2, 1.2, Math.sin(angle)) * 12;
  // Push the north-east out for the lighthouse headland.
  radius += smoothstep(0.2, 1.0, Math.cos(angle - 0.9)) * 14;
  return clamp01((radius - d) / 26);
}
...
function baseElevation(x: number, z: number): number {
  ...
  if (mask <= 0) {
    ...
    const d = Math.hypot(x, z);
    const drop = smoothstep(0, 40, d - 86);
  ...
  const ridge = smoothstep(-20, -66, z) * (11 + fbm2D(x * 0.02, z * 0.02, 3, SHAPE_SEED + 5) * 9);
  // The lighthouse headland is higher still.
  const headland = smoothstep(26, 6, Math.hypot(x - 52, z - -58)) * 8;

  // Beaches: the last few metres of land taper to the waterline.
  const shore = smoothstep(0, 0.32, mask);

  return (rolling + detail + ridge + headland) * shore + mask * 0.9;
}
...
function applyPaths(x: number, z: number, height: number): { height: number; surface: Surface | null } {
  ...
  for (const path of PATHS) {
    ...
    if (core > best) {
      best = core;
      if (core > 0.45) surface = path.surface;
    }
  }
  return { height: result, surface };
}

/** Average of the surroundings — used to level paths without flattening them fully. */
function sampleSmoothed(x: number, z: number): number {
  const r = 4;
  let sum = 0;
  for (let i = 0; i < 4; i++) {
    const a = (i / 4) * Math.PI * 2;
    sum += baseElevation(x + Math.cos(a) * r, z + Math.sin(a) * r);
  }
  return sum / 4;
}
...
export function sampleSurface(x: number, z: number): SurfaceSample {
  ...
  if (flat.surface && height >= 1.0) surface = flat.surface;
...
export const LANDMARKS: Record<string, { x: number; z: number; label: string }> = {
  ...
  'townhall.front': { x: 0, z: -55, label: 'Town Hall' },
  ...
  'beach.pier': { x: 14, z: 47, label: 'Harbour Pier' },
  'beach.log': { x: -4, z: 41, label: 'Driftwood Log' },
  'beach.dunes': { x: -18, z: 38, label: 'Dunes' },
  'lighthouse.point': { x: 52, z: -58, label: 'Lighthouse Point' },
  'meadow.high': { x: -52, z: -46, label: 'High Meadow' },
  'farm.terrace': { x: -40, z: 26, label: 'Garden Terrace' },
  'grove.west': { x: -66, z: 8, label: 'West Grove' },
  'orchard.secret': { x: 62, z: 30, label: 'Secret Orchard' },
};
